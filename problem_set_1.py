...
def bark():
  ...
  capitalize1 = name.capitalize()
  capitalize2 = breed.capitalize()
  print(f'{capitalize1}, the {age} year old {capitalize2} says, "Woof!"')
...
def bark_with_validation():
  ...
  if not name.isalpha() or len(name) < 2: 
    ...
    if not age.isdigit() or not (0 <= int(age) <= 15): 
      ...
      valid_breed = ["Schnauzer", "Terrier", "Poodle", "Mastiff"]
      if breed.capitalize() not in valid_breed:
        print("Breed error!")
      else: 
